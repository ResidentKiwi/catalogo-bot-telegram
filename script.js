...
async function carregarCanais() {
  try {
    ...
    canais.forEach(c => {
      ...
      card.innerHTML = `
        <h5>${c.nome}</h5>
        <p class="mb-1"><strong>Categoria:</strong> ${c.descricao || "N/A"}</p>
        <a href="${c.url}" target="_blank" class="btn btn-primary btn-sm">
          <i class="fas fa-paper-plane"></i> Acessar Canal
        </a>
        ${c.imagem
          ? `<img src="${c.imagem}" class="image-preview mt-3" alt="Imagem do canal">`
          : ""}
      `;
...
document.getElementById("add-channel-form").addEventListener("submit", async (e) => {
  ...
  const descricao = document.getElementById("categoria").value;
